...
@section('content')
    ...
    <section class="m-4 max-lg:mt-14 p-4 bg-white shadow-lg rounded-md">
        <div class="flex items-start justify-center max-md:items-start max-md:justify-start flex-col flex-wrap">
            ...
            <form class="flex flex-wrap justify-start items-start gap-4 my-4" method="GET"
                action="{{ route('patient_list') }}">
                <input class="max-md:text-sm  max-md:py-1 max-md:px-2 border border-gray-400 py-2 px-7 rounded-md"
                    type="text" name="search" placeholder="Search...">
                <select
                    class="max-md:text-sm max-md:py-1 max-md:px-2 border border-gray-400 py-2 px-7 text-center rounded-md"
                    name="sort">
                                ...
                                <a
                                    ...
                                    @else
                                        <form action="{{ route('restore.patient', $patient->id) }}" method="POST">
                                            ...
                                            <button class="py-2 px-4 max-md:py-1 max-md:px-2" type="submit">
                                                <img class="h-5 sm:h-4 sm:w-4 max-md:h-4 max-md:w-4 max-md:hidden"
                                                    src="{{ asset('assets/images/restore.png') }}"
                                                    alt=""></button>
                                        </form>
                                    @endif
                                </a>
